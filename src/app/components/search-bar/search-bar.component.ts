...
export class SearchBarComponent implements OnInit {
  ...
  setDates(): void {
    ...
    for (let i = 0; i < diff; i++) {
      ...
      const month =
        JSON.stringify(from_Date.getMonth() + 1).length === 1
          ? `0${from_Date.getMonth() + 1}`
  ...
  submit(): void {
    this.searchService.addSearch(
      this.getDateByControlValue(this.search.controls.fromDate.value),
      this.getDateByControlValue(this.search.controls.toDate.value)
    );
    this.setDates();
  }
  ...
  getDateByControlValue(control: string): string {
    ...
    const month =
      JSON.stringify(controlDate.getMonth() + 1).length === 1
        ? `0${controlDate.getMonth() + 1}`
